...
public class ContextUtil {
    ...
    protected static Context trueEnter(String name, String origin) {
        ...
        // threadLocal没有context,尝试从缓存中获取
        if (context == null) {
            // 缓存map的key为context名称，value为EntranceNode
            Map<String, DefaultNode> localCacheNameMap = contextNameNodeMap;
            // 获取EntranceNode，双重检测锁，防止并发创建
                    ...
                    try {
                        ...
                        if (node == null) {
                            if (contextNameNodeMap.size() > Constants.MAX_CONTEXT_NAME_SIZE) {
                                ...
                                // Add entrance node.新建的node添加到ROOT
                                Constants.ROOT.addChild(node);
                                // 新建node写入到缓m存map
                                // 为了防止迭代稳定性问题 - iterate stable,对于共享集合的写操作
            ...
            // 将context的name与entreanceNode封装为context
